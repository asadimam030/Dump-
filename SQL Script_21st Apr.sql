...
select abs(rent) as abs from table_name;
...
use world;
    ...
    # If
select Name, Continent, if((population/surfaceArea) > 410, "Densly Populated","Not Densly Populated") 
as Pop_Status from country
having Pop_status = "Densly Populated";

select Name, Continent, if (population/surfaceArea > 410, "Densly Populated","Not_Densly_Populated") as Pop_status from country
  having Pop_status = "Densly Populated";

select avg(population/surfaceArea) as Avg_Pop_den from country;
    # Case When Operator
select * from city
order by Population desc;

select Name,countrycode, case
when Population > 8000000 then "Very_High"
when Population <= 8000000 and Population > 6000000 then "High"
when Population <= 6000000 and Population > 3000000 then "Medium"
else "Low" end as Pop_category
from city
order by Pop_Category;  

select Name, countrycode, Population, case 
when Population > 8000000 then "Very High"
when Population between 6000000 and 8000000 then "High"           
when Population between 3000000 and 6000000 then "Medium"
else "Low" end as Population_Category
from city
order by Population_Category desc;

select name,indepyear, coalesce(indepyear,null,"NA") as new_name from country;
...
select name,Indepyear, coalesce(cityIndepyear,null,"NA") as New_Name from country;

select name, IndepYear, coalesce(IndepYear,null,"NA") as New_Name from country;

    # Nullif
select nullif(exp1, exp2) as something from table_name;
use sakila;
select * from rental;
select rental_date,return_date,nullif(date_add(rental_date, interval 4 day),return_date) as Comp from rental
having Comp is null;
...
use world;
select * from country;
select Name, Indepyear, ifnull(Indepyear,"NA") as Corrected_Indep_Year from country;

    
## Window Functions
    # Rownumber
select Name,Continent, GNP, 
row_number() over(order by GNP desc) as Ranking from Country;

# Rank and Dense Rank
select Name,Continent, GNP, 
Rank() over(order by GNP desc) as Ranking, 
dense_rank() over(order by GNP desc) as Dense_Ranking 
from country;
    
    # Ntile
select Name,Continent, GNP, 
Ntile(5) over(order by GNP desc) as Clusters from country; 
...
lag(sales) over(order by year) as Alias;

select Name, Year,Sales,
lead(sales) over(order by year) as Alias from table_name;
    # Sum()over()
    
select Name, Year,Sales,
Sum(sales) over(order by year) as Cumulative_Sales from table_name;
    
    # Avg()over()
use sakila;    
### Home-WorK
select customer_id,
count(customer_id) as Times_repeated,
rental_date,
return_date,
date_add(rental_date,interval 1 week)as Target_return_date,
datediff(return_date,rental_date) as Rent_Duration 
from rental
where datediff(return_date,rental_date) > 7
group by customer_id 
order by times_repeated desc, Rent_duration desc;    

select customer_id, count(customer_id) as No_of_penalties 
from rental
where datediff(return_date, rental_date)>7
group by customer_id
...
use sakila;
select customer_id,rental_date,return_date,datediff(return_date,rental_date) as rent_duration from rental 
group by customer_id having datediff(return_date,rental_date) > 7;
...
Select name, countrycode,case
when population > 8000000 then "very_high"
when population between 7000000 and polutaion > 6000000 then "High" 
when population between 5000000 and population > 3000000 then "mediam"
else "low" end as population_category
from city
order by population_category desc;
